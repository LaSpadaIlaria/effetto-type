...
// ============ FUNZIONE PER AGGIORNARE LA FLUID SIMULATION ============
function updateFluidSimulationForCurrentNode() {
    const totalNodes = 26;
    const currentNode = currentNodeIndex + 1; // 1-based
    
    console.log(`📊 Aggiorno fluid simulation - Nodo ${currentNode}/${totalNodes}`);
    
    // Usa la funzione globale controlFluidEffect se esiste
    if (window.controlFluidEffect) {
        window.controlFluidEffect(currentNodeIndex, totalNodes);
    } else {
        // Fallback: modifica direttamente l'opacità del canvas
        const fluidCanvas = document.getElementById('smokey-fluid-canvas');
        if (fluidCanvas) {
            const progress = currentNodeIndex / (totalNodes - 1);
            const opacity = 0.305 * (1 - progress);
            fluidCanvas.style.opacity = opacity;
            
            if (currentNodeIndex === totalNodes - 1) {
                fluidCanvas.style.opacity = '0';
            }
            
            console.log(`🎨 Fallback - Impostata opacità a: ${opacity.toFixed(3)}`);
        }
    }
}
...
function updateMovement() {
    if (movementState === 'MOVING_TO_NODE') {
        ...
        if (distanceToTarget < 0.0005) {
            ...
            // AGGIORNA LA FLUID SIMULATION QUANDO RAGGIUNGI UN NODO
            updateFluidSimulationForCurrentNode();
...
const sketch = (p) => {
    ...
    p.setup = function() {
        ...
        // Aggiorna la fluid simulation per il nodo iniziale
        setTimeout(() => {
            updateFluidSimulationForCurrentNode();
        }, 1000);
